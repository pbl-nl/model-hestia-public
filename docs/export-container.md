...
---
# Instructie voor het maken van een exportcontainer in Hestia
...
De oplossing hiervoor is het creëren van een container die per woning inzicht biedt (domein = BO) voor de onderstaande gewenste overzichten:
...
Bij het aanmaken van een container worden accolades gebruikt om de inhoud van de container, zoals te zien is in de bovenstaande syntax.
...
1. De eerste item op de lijst is de evaluatie of een woning een bouwdeel en/of een actief product heeft. De eerste vier attributen in de container zijn gekoppeld aan deze evaluatiecriteria.
...
2. De tweede item geeft aan uit hoeveel unieke isolatieambities (unieke isolatiepakketten) een woning kan kiezen. 
	
	In totaal zijn er een enorme hoeveelheid combinaties die in theorie genomen zouden kunnen worden maar om de 		looptijd van een modelrun te beperken is er een voorselectie gemaakt. Hierdoor zijn er vijf relevante sets 		toepasbaar. De niveaus zijn:
...
3. Net als bij de tweede item op de lijst wordt ook bij de derde item de `count`-functie gebruikt om het aantal geschikte opties te tellen. Dit betreft een uniek isolatiepakket met een installatie, gespecificeerd per categorie (ketel, hybride, all-electric).
...
4. Bij de vierde item wordt beoordeeld of een woning daadwerkelijk een investering doet. De uitkomst is wederom **True** of **False**, waardoor de **value-unit** voor deze attribuut ook wordt ingesteld als `bool`.

5. Bij de eerste item werd aangegeven dat de parameter wordt gevolgd door een de domein-unit (BO) zodat het duidelijk wordt naar welk domein er wordt gerefereerd in de attribute. Voor deze item op de lijst is de uitkomst van de gekozen isolatie ambitie uitkomst in een andere container wat in een ander domein zit en dus ook een andere dimensie heeft dan het BO domein. Daardoor wordt het codeblok aangepast door `org_rel` te gebruiken wat hieronder is weergegeven. org_rel geeft de relatie van het domein waar het resultaat in zit naar het domein waar de informatie verzameld moet worden. Als gevolg hiervan is de domein-unit niet meer nodig achter de parameter naam.
...
container InvesteringenSamenvatting {
		...
		{
		{
			attribute<bool> ProductActief 			 := Activatie/ProductActief[org_rel];
			attribute<bool> BouwdeelActief 			 := Activatie/BouwdeelActief[org_rel];
			attribute<bool> BouwdeelOfProductActief	 := ProductActief || BouwdeelActief;
			attribute<bool> BouwdeelEnProductActief  := ProductActief && BouwdeelActief;
		
			attribute<uint32> 	AantalUniekeAmbities 		 := count(BouwdeelActieveWoning/UniekAmbitie/Bo_rel, BouwdeelActieveWoning/UniekAmbitie/Bo_rel)[org_rel];
			attribute<uint32> 	AantalInvesteringsopties 	 := count(ActieveWoning/xInvesteringsOptie/Bo_rel, ActieveWoning/xInvesteringsOptie/Bo_rel)[org_rel], IntegrityCheck = 'ProductActief && BouwdeelActief ? this <= 3 * AantalUniekeAmbities : ProductActief && not(BouwdeelActief) ? this <= 3 :  this <=5', Descr = 'Isolatieambitites x GebouwoptieCategorie';
			attribute<bool> 	InvesteringGedaan			 := any(ActieveWoning/Choice_per_ActieveWoning/isSelected, invert(ActieveWoning/Choice_per_ActieveWoning/xInvesteringsOptie_rel))[invert(ActieveWoning/Bo_rel)[org_rel]];
			attribute<uint32> 	GekozenIsolatieAmbitie   	 := ActieveWoning/Choice_Per_BO/GeschikteOptie_rel[org_rel];
		}
		}
```
...
_De stukken uitleg zijn afkomstig uit het functioneel ontwerp van Hestia 1.0, waar een uitgebreidere toelichting te vinden is.  
